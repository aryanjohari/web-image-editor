...
export function buildAiMoodSystemPrompt(): string {
  const catalogLines = PRESET_CATALOG.map(
    (entry) =>
      `- id: "${entry.id}" | label: ${entry.label} | category: ${entry.category} | keywords: ${entry.keywords.join(", ")} | ${entry.description ?? ""}`,
  ).join("\n");
...
- For portfolio, landing-page, and ambient site-background requests, prefer featured presets (soft-drift, film-grain, night-gradient, archive, soft-bloom, sunset-melt, clean-loop) unless the user explicitly asks for glitch, neon, club, or punk aesthetics.
