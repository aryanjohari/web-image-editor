...
**Lab workspace (post–Phase 5 UI remake + multi-page shell):** `workspace/` IndexedDB (`stage-workspace`) stores multi-brand kits + asset blobs (bytes copied into `Blob`s — not live `File` handles); active brand id in `localStorage`. The **Workspace** page (`/workspace`) owns library CRUD (Brands | Assets). Uploading stores assets only — use **Use as hero** / **Use as overlay** / **Open in Studio** to paint the canvas. **Studio** (`/studio`) is the create surface (full-bleed canvas, floating Brief, soft **Tune** card that fades away while scrubbing Intensity/Motion/Grit, Export menu); `/lab` redirects there. Legacy `brandKitStorage.ts` (`stage.activeBrandKit.v1`) migrates once. Floating Brief sends the active workspace brand into `applyBriefFromText`. Not synced with `/v1/brands`.
