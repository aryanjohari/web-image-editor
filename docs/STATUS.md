...
- **Hotfix (2026-09-01):** Vercel `/api/talk` bundled via esbuild (`api/talk.entry.ts` → `api/talk.js` on prebuild) — redeploy after merge; smoke `POST /api/talk` or Lab “more grain” with `GEMINI_API_KEY` set
